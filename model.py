...
class ICNet(Network):
    ...
    def get_output_node(self):
        if self.mode == 'inference':
            ...
            output_classes = tf.argmax(logits_up, axis=3)  # (1, 1024, 2048)
            output = decode_labels(output_classes, self.o_shape, self.cfg.param['num_classes'])

            # normalize to [0, 1] to represent probability
            logits_up_proba = tf.nn.softmax(logits_up, axis=3)
            proba = tf.reduce_max(logits_up_proba, reduction_indices=[3])  # (1, 1024, 2048), get max probability

            self.output_classes = tf.reshape(output_classes, (self.cfg.INFER_SIZE[0], self.cfg.INFER_SIZE[1]))  # get classes
            self.proba = tf.reshape(proba, (self.cfg.INFER_SIZE[0], self.cfg.INFER_SIZE[1]))  # get proba
    ...
    def predict(self, image):
        return self.sess.run((self.output, self.output_classes, self.proba), feed_dict={self.img_placeholder: image})
